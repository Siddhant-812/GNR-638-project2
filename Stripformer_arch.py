...
class Embeddings(nn.Module):
    def __init__(self):
        ...
        self.en_layer1_1 = nn.Sequential(
            nn.Conv2d(3, 8*MULT, kernel_size=3, padding=1),
            self.activation,
        )
        
        self.en_layer1_2 = nn.Sequential(
            nn.Conv2d(8*MULT, 8*MULT, kernel_size=3, padding=1),
            self.activation,
            nn.Conv2d(8*MULT, 8*MULT, kernel_size=3, padding=1))
        ...
        self.en_layer2_1 = nn.Sequential(
            nn.Conv2d(8*MULT, 16*MULT, kernel_size=3, stride=2, padding=1),
            self.activation,
        )
        
        self.en_layer2_2 = nn.Sequential(
            nn.Conv2d(16*MULT, 16*MULT, kernel_size=3, padding=1),
            self.activation,
            nn.Conv2d(16*MULT, 16*MULT, kernel_size=3, padding=1))
        ...
        self.en_layer3_1 = nn.Sequential(
            nn.Conv2d(16*MULT, 32*MULT, kernel_size=3, stride=2, padding=1),
            self.activation,
        )
...
class Embeddings_output(nn.Module):
    def __init__(self):
        ...
        self.de_layer3_1 = nn.Sequential(
            nn.ConvTranspose2d(32*MULT, 16*MULT, kernel_size=4, stride=2, padding=1),
            self.activation,
        )
        head_num = 2
        dim = 16*MULT
        ...
        self.de_layer2_2 = nn.Sequential(
            nn.Conv2d(16*MULT+16*MULT, 16*MULT, kernel_size=1, padding=0),
            self.activation,
        )
        ...
        self.de_layer2_1 = nn.Sequential(
            nn.ConvTranspose2d(16*MULT, 8*MULT, kernel_size=4, stride=2, padding=1),
            self.activation,
        )

        self.de_layer1_3 = nn.Sequential(
            nn.Conv2d(16*MULT, 8*MULT, kernel_size=1, padding=0),
            self.activation,
            nn.Conv2d(8*MULT, 8*MULT, kernel_size=3, padding=1))
        ...
        self.de_layer1_1 = nn.Sequential(
            nn.Conv2d(8*MULT, 3, kernel_size=3, padding=1),
            self.activation
        )
...
if __name__ == "__main__":
    ...
    x = torch.rand(size=(32,3,256,448))
    x = x.to(device=device)
    model = Stripformer()
    model = model.to(device=device)
    torchinfo.summary(model, input_size=(32,3,256,448))
    y = model(x)
    print(y)
